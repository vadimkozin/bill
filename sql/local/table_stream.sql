CREATE TABLE `loc_stream` (
  ...
  `numtpl` varchar(16) NOT NULL DEFAULT '-' COMMENT 'шаблон на отбор номеров  из таблицы из поля Fm',
...
) ENGINE=MyISAM AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='Тарифы местной связи по потоку(loc_stream)';
...
CREATE TABLE `_TABLE_CREATE_` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_cid` (`cid`)
) ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='Тарифы местной связи по потоку(loc_stream)';